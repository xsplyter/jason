...
=== Fine grained conflict specification

// add new support with conflict
If the conflicts happen only among specific plans, Jason provides a series of ways to define this kind of conflict, by means of the *conflict* annotation.

First of all, it is important to say that the conflict relation is symmetric, which means that if one specifies that plan A conflicts with plan B, then plan B will also conflict with plan A.

==== Conflict events

A plan can conflict with all other plans that handle the same event. The conflict is specified by informing with which triggers a plan is conflicting.

----
@alabel[conflict("+!g1(_)")] // this plan conflicts with all plans that handle the adoption of the goal !g1(_)
+!update(X)
   <- -vl(T);   // gets the current value of belief vl
      +vl(T+X). // updates its value
...
If a plan conflict with plans that have different triggers, the list of all triggers can be informed.

----
@alabel[conflict(["+!g1(_)", "+!g2"])] // this plan conflicts with all plans that handle the adoption of the goal !g1(_) or the adoption of goal !g2
+!update(X)
   <- -vl(T);   // gets the current value of belief vl
      +vl(T+X). // updates its value
...
==== Conflict plan labels

If the plan is conflicting just with a certain plan, Jason allows to specify the conflict directly with the label of that specific plan.

----
@alabel[conflict("@updateDataBase")] // this plan conflicts only with the plan with label updateDataBase
+!update(X)
   <- -vl(T);   // gets the current value of belief vl
      +vl(T+X). // updates its value
----
...
The keyword *self* can be used to specify a conflict of a plan with itself. It means that two instances of the same plan cannot be executed concurrently.

----
@alabel[conflict(self)] // just one instance of this plan can be in execution
+!update(X)
   <- -vl(T);   // gets the current value of belief vl
      +vl(T+X). // updates its value
----
...
The unnamed variable *_* can be used to specify a conflict of a plan with all other plans (this is the same as specifying a plan as *atomic*).

----
@alabel[conflict(_)] // this plan conflicts with all other plans, and once this plan is in execution, no other plans will be in execution concurrently with it
+!update(X)
   <- -vl(T);   // gets the current value of belief vl
      +vl(T+X). // updates its value
----

==== Different kind of conflicts together

A list of conflicts can be specified combining the different ways to specify conflicts.

----
@alabel[conflict(["+!g1(_)", "+!g2", "@updateDataBase", self])] // this plan conflicts with all plans that handle the adoption of the goal !g1(_) or the adoption of goal !g2, with the plan labelled as updateDataBase, and with itself
+!update(X)
   <- -vl(T);   // gets the current value of belief vl
      +vl(T+X). // updates its value
----
...
. Maicon R. Zatelli, Jomi F. Hübner, Alessandro Ricci, Rafael H. Bordini. *Conflicting Goals in Agent-oriented Programming*. In: 6th International Workshop on Programming Based on Actors, Agents, and Decentralized Control (AGERE!@SPLASH), 2016, Amsterdam. p. 21-30. DOI: http://dx.doi.org/10.1145/3001886.3001889[10.1145/3001886.3001889]
